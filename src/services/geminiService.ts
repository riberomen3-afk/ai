...
3. 선출(Leads) 작성 규칙 (CRITICAL SYSTEM RULE):
   ...
   - 필수적으로 **"vs 물리 어태커 위주 파티"**와 **"vs 특수 어태커 위주 파티"**, 그리고 **"vs 특정 메인 메가진화 포켓몬 (예: 메가캥카, 메가보만다, 메가팬텀 등)"**, **"vs 특정 날씨 파티 또는 막이 사이클"** 등 구체적이고 다양한 상황에 대한 선출 및 운영법을 반드시 5개 이상 포함하세요.
...
4. 활용 시점, 세부조정 및 파티 기여도(Usage Notes, CRITICAL): 파티의 모든 포켓몬(6마리 전체)에 대해 각각 (1) 실전에서의 활용 시점 및 주요 역할, (2) 노력치 세부조정의 명확한 이유 (특정 타겟 스피드 컷, 타겟 내구 데미지 계산 타겟팅 등), (3) 파티 시너지에 기여하는 점 등을 포함한 상세 활용 팁을 'usageNotes'에 구체적으로 꽉 채워서 작성하세요. (예시: 스피드는 무보정 85족 추월 컷, HP는 먹다남은음식 효율을 위한 16n+1 조정 등) 구체적인 수치와 타겟을 이 항목에 상세히 서술해야 합니다.
...
14. 공격 상성 및 기술 일치 (CRITICAL): **포켓몬의 노력치(EVs) 투자 및 성격은 반드시 해당 포켓몬의 기술 배치(물리/특수)와 일치해야 합니다.** 예를 들어, 공격(Atk)에 노력치를 주거나 고집/명랑 같은 공격 상승 성격을 채용했다면 특수 공격기가 아닌 '물리 공격기'를 배치해야 합니다. 반대로 특수공격(SpA)에 노력치를 주고 조심/겁쟁이 성격을 선택했다면 '특수 공격기'를 배치해야 합니다. 기술 배치와 노력치 분포가 엇갈리는 현상(예: 특수공격에 전량 투자해 놓고 지진/인파이트 등 물리기를 주는 행위)을 절대 금지합니다.
15. 아이템 사용 규칙 (CRITICAL): 포켓몬의 역할과 현재 메타에 기반하여 실전성(competitive viability)을 우선적으로 고려하여 도구를 배분하세요. '생명의구슬(Life Orb)', '돌격조끼(Assault Vest)', '울퉁불퉁멧(Rocky Helmet)' 같은 금지된 도구는 절대 제안하지 마세요. 강력한 에이스는 메가스톤이나 구애스카프, 기합의띠를, 내구 보완이 필요하다면 먹다남은음식, 자뭉열매를 주어야 합니다. 핫삼 등에게 아무런 이유 없이 무조건 "금속코트" 같은 단일 타입 강화 도구만 주는 1차원적인 선택을 피하고, 만약 타입 강화 도구를 부여한다면 반드시 해당 포켓몬이 그 타입의 공격기를 기술 배치에 포함하고 있어야 합니다.
16. 성격명 규정: 성격을 출력할 때는 반드시 괄호 안에 어떤 스탯이 상승하고 하락하는지 명시하세요. (예: "고집(공격 상승/특공 하락)")
17. 사용률 제한 및 다양성 확보 (CRITICAL): 파티 구성 시 반드시 **사용자가 제공한 champs.pokedb.tokyo의 최근 픽률(사용률) 데이터를 철저히 분석하여** 강력하고 많이 쓰이는 포켓몬으로 팀을 구성하세요. 특히 기술 배치, 도구, 성격, 세부 노력치 조정도 해당 사이트의 최신 통계(템플릿) 데이터에 최대한 맞춰서 샘플을 짜야 합니다. (마이너 포켓몬 및 무작위 픽은 절대 채용 금지). 단, **메가진화 포켓몬 선택에 있어서는 항상 똑같은 메가캥카나 메가보만다만 추천하지 말고, 상황과 파티 컨셉에 맞는 새롭고 '다양한 메가진화 포켓몬'들도 적극적으로 섞어서 제안해주세요.**
18. 메가니움 특수 규정 (CRITICAL): 최근 메가니움은 물리형이 아닌 '특수 공격형(Special Attacker) 딜러'로 많이 쓰입니다. 그러므로 '칼춤', '지진', '꽃보라' 같은 물리 기술을 절대로 넣지 말고, '기가드레인', '대지의힘', '문포스' 등의 특수 공격기 위주로 기술을 배치하세요. 또한 메가진화 도구의 이름은 반드시 '메가니움나이트'로 정확히 기재하시고, 메가진화 폼 이름은 '메가메가니움'으로 출력하세요.
19. 로토무 폼 명시 (CRITICAL): 파티에 투입되는 로토무는 워시로토무, 히트로토무, 커트로토무, 스핀로토무, 프로스트로토무 등 폼체인지 형태가 다양합니다. 만약 로토무를 파티에 배치한다면 그냥 '로토무'라고 쓰지 말고 반드시 어떤 폼인지 명확하게 명시하세요.
20. 싱글 배틀 규정 (CRITICAL): 파티는 싱글 배틀을 기준으로 구성됩니다. 따라서 턴을 끄는 형태(스톨 파티 등)의 분명한 목적이 있지 않은 이상, 아무 포켓몬에게나 무의미하게 '방어(Protect)' 기술을 채용하지 마세요. 또한 '열풍(Heat Wave)' 등 더블 배틀에만 주로 쓰이는 광역기나 보조기는 최대한 배제하고, 싱글 배틀에 적합한 단일 타겟팅 고위력기 위주로 구성하세요.
21. 메가진화의 다양성 확보 (CRITICAL): 매번 파티를 짤 때 캥카, 이상해꽃 등의 특정 메가진화 포켓몬만 고집하지 마십시오. 이어롭, 리자몽, 아쿠스타(메가), 플라제스(플라엣테), 메가니움 등 통계 상위권에 속해 있는 다채로운 메가진화 포켓몬들을 적극 활용하여 **다양한 형태의 메가진화 에이스**를 제안해야 합니다. 똑같은 주축의 파티 구성을 반복하지 마세요.
22. 플레이스타일 및 아키타입 맞춤형 제안 (CRITICAL): 사용자가 선택한 아키타입(공격형, 기믹형, 방어형, 날씨팟, 트릭룸 등)에 맞춰 시너지가 확실한 팀을 조립하세요.
  ...
  
  {
    "team": {
      ...
      "leads": [
        ...
        { 
          "strategyName": "vs 메가보만다 주축 굿스태프", 
          "description": "내 선출: 포켓몬2 + 포켓몬5 + 포켓몬6\n운영법: 포켓몬2의 위협으로 드래곤클로를 버티며..." 
        },
        { 
          "strategyName": "vs 뽀록나+야도란 막이 사이클", 
          "description": "내 선출: 포켓몬3 + 포켓몬4 + 포켓몬6\n운영법: 포켓몬3의 랭크업으로 사이클을 부수고..." 
        },
        { 
          "strategyName": "vs 비구름 날씨 파티", 
          "description": "내 선출: 포켓몬1 + 포켓몬5 + 포켓몬2\n운영법: 포켓몬1의 날씨 장악을 통해..." 
        }
      ],
      "usageNotes": [
